"""
Forex Analyzer Pro v2.1 — single-file Streamlit dashboard
...
Run:
    streamlit run forex_analyzer_pro_v21.py
...
APP_VERSION = "v2.1"
DATA_CACHE_VERSION = "forex-analyzer-v2.1-data"

st.set_page_config(
    page_title="Forex Analyzer Pro v2.1",
    page_icon="📈",
    layout="wide",
...
def format_price(value):
    """Format prices for easy reading without scientific notation."""
    try:
        value = float(value)
...
def signal_box(signal):
    """Show BUY/SELL/NEUTRAL as a high-visibility badge."""
    styles = {
        "BUY": ("#0b7a3b", "🟢 BUY"),
        "SELL": ("#c62828", "🔴 SELL"),
        "NEUTRAL": ("#6b7280", "⚪ NEUTRAL"),
    }
    bg, label = styles.get(signal, ("#6b7280", signal))
    st.markdown(
        f"""<div style="background:{bg};color:white;padding:14px 18px;border-radius:12px;
        text-align:center;font-size:28px;font-weight:800;letter-spacing:1px;margin:4px 0 10px 0;">
        {label}
        </div>""",
        unsafe_allow_html=True,
    )
...
fig.update_layout(
    title=f"{asset_name} • {tf_label} • Yahoo Finance {symbol} • Last {format_price(latest_close)}",
    height=850,
...
r1.metric("Entry", format_price(entry))
r2.metric("Stop Loss", format_price(sl))
r3.metric("Take Profit", format_price(tp))
...
with st.expander("📋 Signal History / Raw Data"):
    ...
    display = view[cols].tail(100).sort_index(ascending=False).copy()

    # Make price/indicator numbers readable on phones and prevent scientific notation.
    price_cols = [c for c in ["Open", "High", "Low", "Close", "Support", "Resistance"] if c in display.columns]
    for c in price_cols:
    ...
    st.dataframe(
        ...
        column_config={
            **{c: st.column_config.NumberColumn(c, format="%,.2f") for c in price_cols},
            **{c: st.column_config.NumberColumn(c, format="%,.4f") for c in [f"EMA{ema_fast}", f"EMA{ema_slow}", f"EMA{ema_trend}"] if c in display.columns},
